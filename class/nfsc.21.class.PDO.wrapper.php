...
*
*	+------------------------------------------------------------------------------------------------------------------+
*   | Tabela 3 - codigo da classificacao do item (ref. 11.5)                                                           |
*	+----------------------+--------+----------------------------------------------------------------------------------+
...
*   |                      | 0104   | Assinatura de serviços de provimento à internet                                  | <<-- provedores de internet
...
*	+----------------------+--------+----------------------------------------------------------------------------------+
*   | 05. Outros Serviços  | 0501   | Serviço Adicional (substituição de número, troca de aparelho, emissão de 2ª via  |
*   |                      |        | de conta, conta detalhada, etc.)                                                 |
*   |                      | 0502   | Serviço Facilidades (identificador de chamadas, caixa postal, transferência      |
...
*   |                      | 0603   | Energia Elétrica - Serviço (Vistoria de unidade consumidora, Aferição de Medidor,|
...
class Nfsc_21
{
    ...
    public function Mestre($arrayMESTRE, $data1_1, $nf_numero, $nf_ref_item, $data_apuracao, $data_emissao, $dados_empresa, $modelo, $tipo_utilizacao, $database)
    {
                    ...
                    if ($first_count_item == 0)
                        ...
                        switch ($cipnf['@TotalItemsPerNF']) {
                            ...
                            case 3:
                                ...
                                break;
                            default:
                                print "<pre>erro: limite de 3 items por documento fiscal.</pre>";
                                break;
                        }
        ...
        if (!@file_put_contents('Files/001/' . $this->file_001, $this->layout_001, LOCK_EX))
            throw new Exception('O arquivo MESTRE <b>' . $this->file_001 . '</b> n&atilde;o pode ser escrito!');
        else {
    ...
    public function Cadastro($arrayMESTRE, $nf_numero, $nf_ref_item, $data_apuracao, $data_emissao, $dados_empresa, $modelo, $database)
    {
        ...
        foreach ((array) $arrayMESTRE as $valueMESTRE) {
            ...
            #02 - X - tratar IE cliente - ausencia da informacao preencher com ISENTO seguido de posicoes em branco
            //$this->cliente_ie = "231.135.384";
            $this->cliente_ie = "00009000000000"; // 00009043528537
            $this->cliente_ie = $valueMESTRE['@ClientIE']; //"00000000000000";
